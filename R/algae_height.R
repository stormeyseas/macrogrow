...
#' * `h_max`, maximum species height. Can be `NA`. If only `h_max` is supplied then algae height = `h_max`
#' * `h_a`, `h_b` and `h_c`, parameters governing height change with `N_f`
#'
#' @details
#' Calculates macroalgae height as \eqn{h_m = \left(\frac{N_f}{h_a}\right)^{h_b} + h_c} up to a maximum of `h_{max}`. 
